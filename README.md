...
- KB chunks：`2117`
- 即刻 chunks：`1845 / 1848`
- AIHOT chunks：`272 / 272`
- embeddings：约 `2116`
- 飞书文档镜像已同步即刻收藏：`1262`
- 最新入库内容时间：`2026-05-15T01:30:09.354Z`
- 最新日报：`2026-05-14`，状态 `success`，飞书 webhook 已补发成功
...
- 日报生成支持 LLM 失败兜底：APIMart/LLM 临时不可用时会退回基础摘要模板，避免整份日报失败。
- 飞书群 webhook 通知链路可用。
...
其他项目需要查询这份本地知识库时，推荐直接调用本项目 CLI，不要直接读写 SQLite。

从任意项目中执行：

```bash
(cd /Users/dahuang/CascadeProjects/knowledge-auto-update && python3 -m jike_collection ask "我之前收藏过 Claude Code 相关内容吗")
```
...
```bash
(cd /Users/dahuang/CascadeProjects/knowledge-auto-update && python3 -m jike_collection ask "最近 AIHOT 里 OpenAI 有什么动态" --source aihot)
(cd /Users/dahuang/CascadeProjects/knowledge-auto-update && python3 -m jike_collection ask "我收藏过哪些 SEO 外链相关内容" --source jike)
```

这样做可以复用本项目的 `.env`、token、SQLite 路径和检索逻辑，也能避免其他仓库误改 `data/jike_collection.db`。
...
如果 LLM 已配置但临时返回 5xx 或通道不可用，日报会自动退回基础摘要模板，并在摘要里标记本次使用了 fallback。
